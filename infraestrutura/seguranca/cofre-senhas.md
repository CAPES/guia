...
| `banco_dados` e seus tipos:<br>`elasticsearch`<br>`mssql`<br>`mysql`<br>`oracle`<br>`postgres`<br>`sybase` | Segredos das bases de dados. |  Gerente de Banco de Dados (RW)<br>Equipe de Banco de Dados (R)<br><br>Equipe API CICD (Contas de integração) (R)<br>Equipe API CM (Contas de integração) (R)<br><br>Equipe de Linux (R) <br><br>Equipe de Segurança (R)<br><br>Equipe de Desenvolvimento (R - **somente para DHT**)|
